#!/usr/bin/env python3
"""
Ordner → DB → DeepFace → Ergebnisse in DB

Ablauf:
1) Alle Bilder aus IMAGE_FOLDER in die Tabelle `Images` importieren (falls noch nicht vorhanden).
2) Für alle Images ohne Ergebnis dieses Modells: DeepFace für Person Detection nutzen.
...
Anpassbar per Umgebungsvariablen:
  ...
  MODEL_NAME (Default 'deepface'), MODEL_VERSION, MODEL_FRAMEWORK
  BATCH_SIZE (Default 64)
  DETECTOR_BACKEND (retinaface, opencv, ssd, dlib, mtcnn, yolov8, yunet, fastmtcnn)
...
MODEL_NAME = os.getenv("MODEL_NAME", "deepface")
MODEL_VERSION = os.getenv("MODEL_VERSION", "latest")
MODEL_FRAMEWORK = os.getenv("MODEL_FRAMEWORK", "DeepFace")
DETECTOR_BACKEND = os.getenv("DETECTOR_BACKEND", "retinaface")  # retinaface, opencv, ssd, dlib, mtcnn, yolov8, yunet, fastmtcnn

BATCH_SIZE = int(os.getenv("BATCH_SIZE", "64"))
ALLOWED_EXT = (".jpg", ".jpeg", ".png", ".bmp", ".tiff")

# =========================
# DB & Schema (same as Gemma3)
# =========================
CREATE_TABLES_SQL = [
    # Images
    """
...
# DeepFace Inferenz
# =========================
def load_image_cv2(path_or_url: str) -> np.ndarray:
    """Lädt Bild als OpenCV-Array"""
    ...
    if img is None:
        raise ValueError(f"Could not load image from {path_or_url}")
    return img

def deepface_detect_faces(image_path: str) -> dict:
    """
    Nutzt DeepFace.extract_faces() um Gesichter zu detektieren.
    Gibt strukturiertes JSON zurück analog zu Gemma3.
    """
    try:
        # DeepFace extract_faces gibt Liste von erkannten Gesichtern zurück
        faces = DeepFace.extract_faces(
            img_path=image_path,
            detector_backend=DETECTOR_BACKEND,
            enforce_detection=False,  # Kein Fehler wenn keine Gesichter
            align=False
        )
        
        person_count = len(faces) if faces else 0
        has_person = person_count > 0
        
        # Confidence schätzen basierend auf Anzahl gefundener Gesichter
        if person_count == 0:
            probability = 0.1  # Niedrige Wahrscheinlichkeit, aber nicht 0
        elif person_count >= 1:
            probability = 0.95  # Hohe Confidence bei gefundenen Gesichtern
        
        notes = f"Detected {person_count} face(s) using {DETECTOR_BACKEND} backend"
        ...
        return {
            ...
            "notes": notes,
            "faces_data": len(faces) if faces else 0  # Zusätzliche Info
        }
        ...
        return {
            ...
            "person_count": 0,
            "probability": 0.5,
            "notes": f"Detection failed: {str(e)[:100]}",
            "faces_data": 0
        }
...
def main():
    ...
    try:
        ...
        print(f"[INFO] Verwende DeepFace mit {DETECTOR_BACKEND} backend.")
        ...
        while True:
            ...
            for image_id, blob_path in todo:
                try:
                    ...
                    prediction = deepface_detect_faces(blob_path)
                    t1 = time.perf_counter()
                    latency_ms = int((t1 - t0) * 1000)

                    method_desc = f"DeepFace face detection with {DETECTOR_BACKEND} backend"
                    ...
                    insert_error(conn, "DeepFace", f"image_id={image_id} path={blob_path} infer_error={repr(ex)}")
